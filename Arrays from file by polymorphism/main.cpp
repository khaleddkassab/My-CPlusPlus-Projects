...
public:
    ...
    virtual void Add (float x)
    {
        for (int i=0;i<Size;i++)
        {
            if(Arr1[i] == -100000)//add i element then compare with it the " float x"
            {
                Arr1[i] = x ;
                return ;
            }
...
class FrontArray:public FloatArray
{
  public:
      ...
      virtual void Add (float x )
      {
          for (int i=0;i<Size;i++)
          {
                if(Arr1[0]==-100000)//to add in the first index after shifting all elements to right
                {
                    Arr1[0]=x;
                    return;
                }

                if (Arr1[0]!=-100000)//if first index not empty shift all elements
                {
